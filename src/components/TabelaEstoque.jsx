export default function TabelaEstoque({ materiais, aoRemover, podeEditar, usuarioLogado }) {
    ...
    <table>
      ...
      <tbody>
        {materiais.length === 0 ? (
          ...
          materiais.map(item => {
            const eDonoDoItem = item.cadastradoPor === usuarioLogado?.nome;
            const permitirAcao = podeEditar && eDonoDoItem;
                ...
                {podeEditar && (
                  <td style={{ textAlign: 'center' }}>
                    {permitirAcao ? (
                      ...
                      <span title={`Cadastrado por: ${item.cadastradoPor || 'Desconhecido'}`} style={{ fontSize: '11px', color: '#999', cursor: 'help' }}>
                        🔒 Bloqueado
                      </span>
                    )}
                  </td>
